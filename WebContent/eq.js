	...
	if (searchLocation === "" || searchLocation == null) {
	   ...
	   url = "https://maps.googleapis.com/maps/api/geocode/json?address=" 
		...
		if ((xmlHttpYear.readyState == 4) && (xmlHttpYear.status == 200))
		{
			...
			alert ("Cannot access Google Geocode Service. Please check network condition.");
			return;
		}
		
	}
	
		// http://140.137.218.59:8080/EarthquakeServer3/GetFromForm3?"'
		...
		xmlHttpYear.onreadystatechange = PostProcessing;
...
function PostProcessing() {
		...
		{
			alert("Cannot access the Earthquake data server.  Please check the network condition.");
			return;
		}
		*/
}
...
function drawMap(result) {
   ...
   // Added to test animation effect
	...
	if (eq[1] === "")
	 ...
	 alert("There is no data that meets the search criteria.");
...
function initMap()
{
	...
	var overlay = new google.maps.GroundOverlay(
		      'http://140.137.218.59:8080/EarthquakeServer3/present.png',
		      imageBounds);
	overlay.setMap(map);
